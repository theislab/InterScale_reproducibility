...
# %%
adata.uns['Layer_color'] = CELL_TYPE_COLORS
...
# %%
adata = sc.read_h5ad('/dss/dssfs03/tumdss/pn36po/pn36po-dss-0002/di93tig/Projects/A3_InterScale/data/chen_adata_pp_hvg.h5ad')
...
df = df.merge(
    adata.obs[["patientID", "stage"]].drop_duplicates(),
    on="patientID"
)
...
plt.tight_layout()
plt.savefig(f"{project_root}/figures/{DATA}/data_visualization/nr_counts_per_layer_per_patient.png", format="png", bbox_inches="tight", dpi=300)
plt.show()
...
# %%
color_list = [CELL_TYPE_COLORS[ct] for ct in adata.obs["Layer"].cat.categories]
adata.uns[f'{"Layer"}_colors'] = color_list
...
sq.pl.spatial_scatter(
    ...
    img=False,
    title=titles,
    save=f"{project_root}/figures/{DATA}/data_visualization/spatial_slides.png"
)
...
sq.pl.spatial_scatter(
    ...
    img=False,
    color=["Layer"],
    save=f"{project_root}/figures/{DATA}/data_visualization/spatial_connectivity_{patient_ID}.png",
    dpi = 300
)

# %%
subadata = adata[adata.obs['patient_id'] == '2-3']
subadata.uns['sliding_window_assignment_colors'] = ["#404040","#bfbfbf", "#bfbfbf",  "#404040"]
# Plot to see if it still makes sense
sq.pl.spatial_scatter(
    subadata,
    spatial_key='spatial',
...
# %%
from InterScale.pp import sliding_window
...
counts = adata.obs.groupby(['stage', 'Layer']).size().unstack(fill_value=0)
...
plt.tight_layout()
plt.savefig(f"{project_root}/figures/{DATA}/data_visualization/Layer_relative_abundance_per_condition.png", format="png", bbox_inches="tight", dpi=300)
plt.show()

# %%
# according to split: Train, Val, Test
patientID_order = [
    "1-1", '18-64', "2-5",  # control
    '2-3', '2-8', 'T4857' # AD
]

counts = adata.obs.groupby(['patientID', 'Layer']).size().unstack(fill_value=0)

# Reorder by setting index as categorical and sorting
counts.index = pd.Categorical(counts.index, categories=patientID_order, ordered=True)
...
plt.legend(title='Layer', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig(f"{project_root}/figures/{DATA}/data_visualization/Layer_relative_abundance_per_patient.png", format="png", bbox_inches="tight", dpi=300)
plt.show()
